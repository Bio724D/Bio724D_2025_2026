...
bind_cols(x, y)                # columns in the result appear in the order provided 
                               # columns with the same name are renamed

# best practice is to provide distinct names prior to the merge
y <- rename(y, M = A, N = B)   # syntax is: new name = old name
bind_cols(x, y)

# before merging, check that both data frames contain the same number of rows
z <- y[1:2, ]                  # make a table with fewer rows; note the comma!
bind_cols(x, z)                # throws an error
# Can't recycle usually means you are trying to do something with vectors of different length
```
...
bind_rows(x, y)                # rows in the result appear in the order provided

# to keep track of which rows came from which original data frame
bind_rows(x, y, .id = "df")    # you can use any name for this new column
...
y <- rename(x, A1 = A)         # change the name column A in x
bind_rows(x, y)                # the column is now split, with many unwanted NAs

# row merges tolerate a different number of columns
x <- mutate(x, D = C * 3)      # make a new column in x
bind_rows(x, y)                # missing values are replaced by NAs
```
...
extras <- bind_cols(penguins, penguins_extras)       # what went wrong?
nrow(penguins)
nrow(penguins_extras)                                

# go back to the original data set
extras <- bind_cols(penguins, penguins_extras_full)  # now it works
View(extras)

# second, a row merge
more <- bind_rows(penguins, more_penguins)           # what happened?
```
...
left_join(x, y, by = "A")      # B is no longer a join column and the result is different

# to be clear about what you want, specify A and B as join columns
left_join(x, y, by = c("A", "B"))      # same result as the first left join, but clearer
```
