...
* Create a data frame from the `crypto_tecan_run_example.txt` file
  ...
  * Challenging: Give the columns meaningful names

    * Since there are 98 columns it would be tedious, error prone, and inefficient to create all of these by hand.   Solve this with computation. The functions `rep` and `str_c` are useful here.
