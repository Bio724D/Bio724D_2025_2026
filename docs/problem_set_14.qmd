---
title: "Problem Set 14: Introduction to Unix"
author: "Greg Wray"
date-format: iso
format: 
  html:
...
4. Practice getting help with another Unix command: `tr`. Use the `man` page to learn how this command can be useful for processing text files. 
...
* Next, use your understanding of `tr` to process the `VoyageOfTheBeagle.txt` file. Show the command that uses `tr` to convert every instance of `Beagle` to `BEAGLE`.  

* Show the command that uses `tr` to delete every instance of `tortoise`. 
